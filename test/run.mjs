...
for (const file of ['data.js', 'okb.js', 'accounts.js', 'import.js', 'store.js', 'doc.js']) {
...
function sample() {
  ...
  c.procedure = 'realization';
  c.caseStartDate = '2025-12-01';
  c.judicialAct = 'Решением Арбитражного суда города Москвы от 12.03.2026';
  c.creditorsSum = '15000000';

  // Должник всегда гражданин: приложение работает только по банкротству
  // физических лиц.
  const debtor = S.debtorOf(c);
  debtor.fio = 'Пшихачева Асият Арсеновна';
  debtor.birthDate = '1975-06-14';
  debtor.inn = '263500123456';
  debtor.snils = '123-456-789 00';
  debtor.address = 'г. Москва, ул. Полевая, д. 1, кв. 5';
...
group('Переменные и подстановка');
{
  ...
  eq('должник', v.DEBTOR_NAME, 'Пшихачева Асият Арсеновна');
  eq('контрагент кратко', v.COUNTERPARTY_SHORT, 'ООО «Василёк»');
  eq('реквизиты должника', v.DEBTOR_REQUISITES, 'ИНН 263500123456, дата рождения 14.06.1975');
  eq('управляющий из профиля', v.MANAGER_NAME, 'Иванов Иван Иванович');
  eq('статус управляющего', v.MANAGER_ROLE, 'финансовый управляющий');
  eq('статус в творительном падеже', v.MANAGER_ROLE_INS, 'финансовым управляющим');
  ...
  eq('шаблон из ТЗ', S.render(tpl, v, false),
    '15 апреля 2025 года между Пшихачева Асият Арсеновна и\n' +
...
group('Условная логика');
{
  ...
  ok('поле дела', S.evalCondition('case.procedure = realization', ctx()));
...
group('Склонение ФИО');
{
  ...
  // Должником организация не бывает, а вот ответчиком — сплошь и рядом.
  const { db, c, d } = sample();
  eq('у организации склоняется только форма',
    D.genitiveOrg('Общество с ограниченной ответственностью «Ромашка»'),
...
/* ================= счета должника ================= */
/*
 * Проверяем ровно то, на чём держится вся затея: ключ проверки счёта,
 * отсев перевёрнутых страниц и то, что в ходатайство идут только
 * отмеченные банки. Образцы строк взяты из настоящих сведений ФНС —
 * с той же кашей вместо кириллицы, какую выдаёт распознавание скана.
 */
{
  group('Счета должника: ключ проверки');

  ok('счёт Сбербанка сходится', A.checkKey('044525225', '40817810600192102302'));
  ok('счёт ТБанка сходится', A.checkKey('044525974', '40817810900005226553'));
  ok('счёт Зенита сходится', A.checkKey('044525272', '40817810410000052385'));
  ok('одна подменённая цифра ломает ключ', !A.checkKey('044525225', '40817810600192102303'));
  ok('чужой БИК не подходит', !A.checkKey('044525974', '40817810600192102302'));
  ok('короткий номер отвергнут', !A.checkKey('044525225', '4081781060019210230'));
  ok('ИНН банка проверяется', A.innValid('7707083893') && !A.innValid('7707083894'));
}

{
  group('Счета должника: разбор сведений ФНС');

  // Страница 1: банк с одним открытым и одним закрытым счётом. Кириллица
  // распознана латиницей по форме букв — так и приходит скан.
  const good = [
    'OO61uiecTBO ¢ orpaHnyeHHoi oTBeTcTBEHHOCTHLIO "XoyM Kpenut"',
    'PerHom/Hom®: 316/ UHH/KIIIT: 7735057951/771401001',
    'BUK(CBU®T): 044525245',
    '40817810893400009119 29.08.2018 09.08.2024 3aKpBIT',
    'ПАО «Сбербанк России»',
    'РегНом/НомФ: 1481/ ИНН/КПП: 7707083893/773601001',
    'БИК(СВИФТ): 044525225',
    '40817810600192102302 31.01.2017 OTKPBIT Texyumii cuet'
  ].join('\n');

  // Страница-перевёртыш: номера счетов распознаны вперемешку с буквами,
  // дат нет ни одной.
  const bad = [
    '19hdHHIIAND|119ddiLo SOTLOFI01090018L180Y',
    '12hdHHITIAND|,119d)10 01€8SCE01097018L180Y',
    '1ohdHHIIANA|,uadyLo 8CS0C6L0109¢018LI8OY'
  ].join('\n');

  eq('читаемая страница', A.pageKind(good), 'good');
  eq('перевёрнутая страница', A.pageKind(bad), 'garbled');
  eq('титул без счетов', A.pageKind('Сведения об открытых и закрытых счетах'), 'plain');

  const r = A.parseFns([{ num: 1, text: good }, { num: 2, text: bad }]);
  eq('банков с открытыми счетами', r.banks.length, 1);
  eq('это Сбербанк', r.banks[0].bik, '044525225');
  eq('ИНН привязан', r.banks[0].inn, '7707083893');
  eq('название прочитано, где кириллица', r.banks[0].name, 'ПАО «Сбербанк России»');
  eq('банк только с закрытыми счетами отложен', r.closedOnly.length, 1);
  eq('закрытых счетов', r.closed, 1);
  eq('о перевёрнутой странице сказано', r.unreadable.join(), '2');

  // Латинская каша названием банка не станет.
  eq('каша не читается как название', A.cleanName('OO61uiecTBO ¢ orpaHnyeHHoi oTBeTcTBEHHOCTHLIO "XoyM Kpenut"'), '');
  ok('строка реквизитов отброшена', !A.cleanName('РегНом/НомФ: 1481/ ИНН/КПП: 7707083893/773601001'));

  eq('открыт', A.accountState(' 31.01.2017 OTKPBIT Texyumii cuet'), 'open');
  eq('закрыт', A.accountState(' 29.08.2018 09.08.2024 3aKpBIT'), 'closed');
}

{
  group('Счета должника: справка об остатке');

  // Табличная справка: остаток стоит между прочерком и служебными кодами,
  // и соседние колонки не должны склеиваться в одно число.
  const table = [
    'Дата открытия Дата закрытия Остаток собственных средств ТБ ОСБ ВСП',
    '08.12.2009 - 47,78 38 7813 1586',
    '14.11.2012 - 38 52 5230 728',
    '07.11.2018 - 5,08 52 5230 728',
    '19.01.2005 - 2,52 38 5278 1476',
    '24.09.2004 - 5,99 38 5278 830',
    '23.01.2017 - 0 52 5230 728'
  ].join('\n');
  const t = A.parseBalance(table);
  eq('сумма по счетам', Math.round(t.total * 100) / 100, 99.37);
  eq('счетов в сумме', t.parts.length, 6);

  const alfa = 'Остаток на начало: 10,25 Обороты по Дт за период: 9 040,50\nОстаток на конец: 0,00';
  eq('остаток на конец периода', A.parseBalance(alfa).total, 0);

  const out = 'Входящий остаток: 40,50\nИсходящий остаток: 20,50\nВходящий остаток: 20,50\nИсходящий остаток: 1 234,56';
  eq('берётся последний исходящий', A.parseBalance(out).total, 1234.56);
  eq('нечего брать — ничего не выдумываем', A.parseBalance('скан без текста').total, null);

  eq('БИК из справки', A.bikFromStatement('БИК 044525593 кор. счёт 30101810200000000593 БИК 044525593'), '044525593');
  eq('банк из имени файла',
    A.nameFromFile('8. Сведения об остатке денежных средств на счетах, открытых в АО Альфа Банк.pdf'),
    'АО Альфа Банк');
}

{
  group('Счета должника: ходатайство');

  const { db, c, d } = sample();
  const st = S.newStatement('petition');
  d.statements.push(st);

  const fns = {
    banks: [
      { bik: '044525225', inn: '7707083893', name: 'ПАО «Сбербанк России»', open: ['40817810600192102302'], closed: [] },
      { bik: '044525974', inn: '7710140679', name: '', open: ['40817810900005226553'], closed: [] }
    ],
    closedOnly: [], closed: 3, unreadable: [4], pages: 6
  };
  eq('заведено банков', S.applyFns(db, c, fns, 'фнс.pdf'), 2);
  eq('название взято из справочника', c.accounts[1].name, 'АО «ТБанк»');
  eq('нечитаемые страницы запомнены', (c.accountsMeta.unreadable || []).join(), '4');

  c.accounts[0].balance = '99.37';
  c.accounts[1].balance = '0';
  eq('итог по счетам', S.accountsTotal(c), 99.37);
  eq('нулевой остаток в тексте не печатается',
    S.accountsBanksText(c), 'ПАО «Сбербанк России» — 99,37 руб., АО «ТБанк»');

  // Снятая галочка убирает банк и из текста, и из приложений.
  c.accounts[1].include = false;
  eq('банк убран из перечня', S.accountsBanksText(c), 'ПАО «Сбербанк России» — 99,37 руб.');
  const att = S.accountsAttachments(c);
  eq('приложений', att.length, 2);
  eq('первое приложение — сведения ФНС', att[0], 'Сведения об открытых и закрытых счетах должника');
  eq('второе — справка Сбербанка', att[1],
    'Сведения об остатке денежных средств на счетах, открытых в ПАО «Сбербанк России»');

  const v = S.context(db, c, d, 'petition');
  eq('переменная банков', v.ACCOUNTS_BANKS, 'ПАО «Сбербанк России» — 99,37 руб.');
  eq('переменная остатка', v.ACCOUNTS_BALANCE, '99,37');
  eq('банков в ходатайстве', v.ACCOUNTS_COUNT, '1');
  ok('приложения ходатайства начинаются со сведений ФНС',
    v.ATTACHMENTS.startsWith('1. Сведения об открытых и закрытых счетах должника'));

  // В заявлении об оспаривании банковских справок быть не должно.
  const vs = S.context(db, c, d, 'statement');
  ok('в заявлении справок банков нет', !vs.ATTACHMENTS.includes('остатке денежных средств'));

  const text = S.documentText(db, c, d, st);
  ok('остаток попал в ходатайство', text.includes('99,37'));
  ok('банк назван в ходатайстве', text.includes('ПАО «Сбербанк России»'));
  ok('исключённый банк не назван', !text.includes('ТБанк'));

  // Справка, разобранная из файла, заводит банк, которого в сведениях ФНС
  // не было: перевёрнутую страницу так и выправляют.
  const added = S.applyStatement(db, c, { bik: '044525593', total: 12.5, method: 'остаток на конец периода' },
    '8. Сведения об остатке денежных средств на счетах, открытых в АО Альфа Банк.pdf');
  eq('банк заведён по справке', added.name, 'АО «Альфа-Банк»');
  eq('остаток из справки', added.balance, '12.5');
